...
TEST(BumpRBF, Center)
{
    ...
    auto rbfExpectedValue =
        std::exp(-1); // TODO: explain why this value is expected
...
TEST(WendlandC0RBF, Center)
{
    ...
    auto rbfActualValue = rbf.evaluate(evaluationDistance);
    auto rbfExpectedValue =
        scalarType{1}; // TODO: explain why this value is expected
    auto tolerance = zlab::evaluate_safe_tolerance();
    EXPECT_NEAR(rbfExpectedValue, rbfActualValue, tolerance);
}
...
TEST(WendlandC0RBF, OutsideSupport)
{
    ...
    auto rbfActualValue = rbf.evaluate(evaluationDistance);
    auto rbfExpectedValue =
        scalarType{0}; // TODO: explain why this value is expected
    auto tolerance = zlab::evaluate_safe_tolerance();
    EXPECT_NEAR(rbfExpectedValue, rbfActualValue, tolerance);
}
