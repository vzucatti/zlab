...
template <typename matrixType>
MGS<matrixType> modified_gram_schmidt(
    ...
    std::optional<scalarType> marginOfError = std::nullopt)
{
    ...
    matrixType Q(numberOfRows, numberOfColumns),
        R(numberOfColumns, numberOfColumns);
    auto V = data.copy();
    for (auto j = 0; j < numberOfColumns; ++j)
    {
        ...
        if (R(j, j) < tolerance)
        {
            throw std::runtime_error(
                "MGS: Matrix is ill-conditioned or rank-deficient.");
        }
