...
static constexpr ButcherTableau<1> ButcherTableauExplicitEuler = {
    {{{0.0}}}, {1.0}, {0.0}};
...
static constexpr ButcherTableau<2> ButcherTableauMidpointMethod = {
    {{{0.0, 0.0}, {0.5, 0.0}}}, {0.0, 1.0}, {0.0, 0.5}};
...
static constexpr ButcherTableau<2> ButcherTableauRalstonsMethod2 = {
    {{{0.0, 0.0}, {2.0 / 3.0, 0.0}}}, {1.0 / 4.0, 3. / 4.0}, {0.0, 2. / 3.0}};
...
class RungeKuttaSolver
{
    ...
    public:
        ...
        auto solve(
            std::function<void(integerType, const ZVector &)> =
                [](integerType, const ZVector &) {});
...
RungeKuttaSolver<numberOfStages, functionType>::RungeKuttaSolver(
    ...
    const ButcherTableau<numberOfStages> &butcherTableau)
    : F(std::move(F)), initialState(initialState.copy()), timeStep(timeStep),
      numberTimeSteps(numberTimeSteps), butcherTableau(butcherTableau)
{
    assert(timeStep > 0);
...
template <positiveIntegerType numberOfStages, typename functionType>
auto RungeKuttaSolver<numberOfStages, functionType>::solve(
    std::function<void(integerType, const ZVector &)> callback)
{
