...
ZMatrix::ZMatrix(ZMatrix &&matrix) noexcept
    : numberOfRows(matrix.numberOfRows),
      numberOfColumns(matrix.numberOfColumns), data(std::move(matrix.data))
{
...
ZMatrix::ZMatrix(
    integerType numberOfRows, integerType numberOfColumns, scalarType fillValue)
...
positiveIntegerType
ZMatrix::compute_vector_index(integerType row, integerType column) const
{
...
ColumnView::ColumnView(ZMatrix &matrix, integerType columnIndex)
    : matrix(matrix), columnIndex(columnIndex)
{
...
ZVector::ZVector(integerType size, scalarType fillValue)
    : matrix(size, 1, fillValue)
{
}
