...
int main()
{
    ...
    // --------------------------------------------------------------------------------
    // PROBLEM: Polynomial Curve Fitting (Cubic: y = c0 + c1*x + c2*x^2 +
    // c3*x^3) Goal: Use Least Squares to recover the known coefficients (c0,
    // c1, c2, c3). The system is overdetermined: A*x = b.
    // --------------------------------------------------------------------------------
    ...
    auto [A, b] =
        vandermondeMatrix_and_targetVector(trueCoefficients, dataPoints);
    ...
    axpy(
        -1.0, trueCoefficients, errorVector); // errorVector -= trueCoefficients
    auto errorNorm = norm(errorVector);

    cout << "\nExpected coefficients:\n";
    for (auto &ci : trueCoefficients)
        cout << ci << " ";
    cout << "\n\nComputed coefficients:\n";
    for (auto i = 0; i < trueCoefficients.size(); ++i)
        cout << computedCoefficients[i] << " ";
    cout << "\n\nCoefficients L2 Error: " << errorNorm << "\n";
    return 0;
}

template <zlab::VectorConcept T>
std::tuple<zlab::ZMatrix, zlab::ZVector> vandermondeMatrix_and_targetVector(
    const T &trueCoefficients, const T &dataPoints)
{
